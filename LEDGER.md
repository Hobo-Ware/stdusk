...
- Repo: `Hobo-Ware/stdusk` (began as a hard fork of Eugeny/tabby). The Rust rewrite is the
  ...
  upstream Eugeny/tabby remains the reference implementation. Historical entries below that say
  "crate in `native/`" or `native/...` paths predate that move.
...
## Build / run / test
```
# crate is at the repo root now (native/ was promoted in 1.0.9)
...
into FUTURE.md (low-hanging fruit + future ideas + deliberately-excluded record; best catch:
OSC 8 hyperlinks, which alacritty_terminal already parses but links.rs ignores).
- **Deleted**: all 15 tabby-* dirs, app/ web/ build/ docs/ locale/ extras/ patches/ scripts/
  snap/ .well-known/ .vscode/, the Electron config set (package.json, yarn.lock, tsconfig,
  webpack*, electron-builder.yml, firebase.json, eslint/pug/mergify/all-contributors/.env),
  the README.*.md translations, CODE_OF_CONDUCT/HACKING, and the Tabby workflows (build.yml,
  docs.yml, release.yml, codeql-analysis.yml). 872 file deletions.
- **Promoted**: git mv of native/* to the root (261 renames). Root .gitignore is now the Rust
  one (/target); LICENSE kept as-is (Tabby MIT - fork attribution obligation).
- **Rewired**: native-release.yml + native.yml stripped of every `working-directory: native`
  / `workspaces: native` / `native/**` path (steps now run at root; dist/, assets/,
  packaging/ are all root-relative). dependabot.yml -> cargo + github-actions at "/". README
  image/link paths, and the forward-looking `cd native` / `native/Cargo.toml` doc
  instructions across README/LEDGER/PLAN/V1/AGENTS/project.md, all corrected.
- **Verified**: cargo build/test (232)/clippy -D warnings/fmt all green from the new root. An
  adversarial sub-agent traced every release-workflow path against `git ls-files` and every
  compile-time embed (include_bytes!/include_str!/build.rs scheme pack) to a tracked root
  file - verdict: the pipeline WILL build on the next tag. target/ confirmed NOT committed.
  main is not branch-protected, so no stale required-check deadlock from the deleted workflows.
...
- **Distribution = Homebrew cask, not formula** (user: "we need it to be findable"). A formula
  ...
  `packaging/stdusk.rb` regenerate the cask.
